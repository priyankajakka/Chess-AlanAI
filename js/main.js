...
function checkStatus2(color) {
    if (game.in_checkmate()) {
        alanBtnInstance.playText("Checkmate!");
        if (game.turn() == 'b') {
            alanBtnInstance.playText("Oops! Looks like computer lost.");
        } else {
            alanBtnInstance.playText("Oops! Looks like you lost.");
        }
    }
    else if (game.insufficient_material()) {
        alanBtnInstance.playText("Draw!");
    }
    else if (game.in_threefold_repetition()) {
        alanBtnInstance.playText("Draw by Threefold Repetition!");
    }
    else if (game.in_stalemate()) {
        alanBtnInstance.playText("Draw!");
    }
    else if (game.in_draw()) {
        alanBtnInstance.playText("Draw by 50 move Rule!");
    }
    else if (game.in_check()) {
        if (game.turn() == 'b') {
            alanBtnInstance.playText("Oops! Looks like computer is in check.");
        } else {
            alanBtnInstance.playText("Oops! Looks like you are in check.");
        }
    }
    else {
        alanBtnInstance.playText("No check, checkmate, or draw. Keep playing!");
    }
}
...
function checkStatus(color) {
    if (game.in_checkmate()) {
        ...
        //$('#status').html(`<b>Checkmate!</b> Oops, <b>${game.turn()}</b> lost.`);
    }
    else if (game.insufficient_material()) {
        alanBtnInstance.playText("Draw!");
        ...
        //$('#status').html(`Oops, <b>${game.turn()}</b> is in <b>check!</b>`);
        return false;
    }
    else {
...
function updateAdvantage() {
    if (globalSum > 0) {
        ...
        alanBtnInstance.playText("It appears that the computer is currently at an advantage.");
    }
    else if (globalSum < 0) {
        /*$('#advantageColor').text('White');
        $('#advantageNumber').text(-globalSum);*/
        alanBtnInstance.playText("It appears that you are currently at an advantage.");
    }
    else {
        /*$('#advantageColor').text('Neither side');
        $('#advantageNumber').text(globalSum);*/
        alanBtnInstance.playText("Currently, neither side is at an advantage.");
    }
}

function getDepth() {
    //var depth = parseInt($('#search-depth').find(':selected').text());
    var depth = blackDepth;
    var proficiency = "";
    if (depth == 1) {
    ...
    alanBtnInstance.playText("The level of this game is " + proficiency);
}

function setDepth(depth) {
    //let element = document.getElementById("search-depth");
    //element.value = depth;
    blackDepth = depth;
}
...
function revealOpeningPositions() {
    $('#openingPos').modal('show');
}

function hideOpeningPositions() {
    $('#openingPos').modal('hide');
}
...
function choosePosition(position) {
    if (position == 'Ruy Lopez') {
        ruyLopez();
    } else if (position == 'italian game') {
        italianGame();
    } else {
        sicilianDefense();
    }
...
function undo2() {
    if (game.history().length >= 2) {
        alanBtnInstance.playText("Undoing move.");
        $board.find('.' + squareClass).removeClass('highlight-white');
        $board.find('.' + squareClass).removeClass('highlight-black');
        $board.find('.' + squareClass).removeClass('highlight-hint');
        ...
        alanBtnInstance.playText("Nothing to undo.");
    }
}
...
function redo() {
    if (undo_stack.length >= 2) {
        alanBtnInstance.playText("Redoing move.");
        ...
        alanBtnInstance.playText("Nothing to redo.");
    }
}
...
function onDrop2(source, target) {
    ...
    // Illegal move
    if (move === null) {
        alanBtnInstance.playText("This is an illegal move. Try again.");
        return 'snapback'
    }
...
function findPossibleMoves(square) {
